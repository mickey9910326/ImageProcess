...
#define HEADER_SIZE 14 // 14 uint8_t bmp-header
#define INFO_SIZE 40
...
typedef struct BMP {
    ...
    UINT32 fileSize; // File size in uint8_ts
...
#define U32(x)    ((int) (x))
#define B2U16(uint8_ts,offset)  (U16(uint8_ts[offset]) | U16(uint8_ts[offset+1]) << 8)
#define B2U32(uint8_ts,offset)  (U32(uint8_ts[offset]) | U32(uint8_ts[offset+1]) << 8 | \
                   U32(uint8_ts[offset+2]) << 16 | U32(uint8_ts[offset+3]) << 24)
